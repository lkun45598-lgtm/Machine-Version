...
REPORT_MD = REPORT_DIR / "实验5-图像特征提取_实验报告.md"
REPORT_DOCX = REPORT_DIR / "实验5-图像特征提取_实验报告.docx"
...
def add_image(document: Document, alt: str, image_path: Path) -> None:
    ...
    if name == "60_all_results.png":
        width = Inches(6.4)
    elif name in {"20_color_channels.png"}:
        width = Inches(5.4)
    elif name in {"33_yellow_compare.png", "41_lbp_compare.png", "54_sift_alignment.png"}:
        width = Inches(6.4)
    elif name in {"52_sift_keypoints.png"}:
        width = Inches(5.4)
    elif name in {"53_sift_match.png"}:
        width = Inches(6.0)
    elif name in {"01_color_original.png", "02_texture_original.png", "04_texture_rotated.png"}:
        width = Inches(3.8)
