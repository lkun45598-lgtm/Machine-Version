...
本实验所用图像为 scikit-image 标准测试集中的 `data.astronaut()`（NASA 宇航员 Eileen Collins 肖像，分辨率 512×512，三通道彩色）。该图像同时包含光滑面（头盔反光面、空气罐表面）、强边缘（航天服轮廓、肩章条纹）和丰富细节（头发、徽章纹理），既便于观察低通滤波后细节的丢失，也便于在高通／边缘结果中辨认轮廓与纹理。FFT 之前先 BGR→灰度，确保频域操作在单通道实数张量上进行。
...
原始彩色输入图（astronaut 标准测试图）转灰度后送入 FFT，输出幅度谱与相位谱：

![幅度谱与相位谱](../data/outputs/12_amp_phase_compare.png)

观察可得：

- 幅度谱在中心附近能量最强，呈现明显的水平／垂直条纹——这是 astronaut 图像中航天服、肩部、头盔等大量水平／垂直边缘在频域上的能量集中。条纹中心的"亮斑"对应于图像整体平均亮度的直流项。
...
- 低通结果只保留了图像中最低的若干个频率分量，整体呈现强烈的"块状模糊"，宇航员面部、头盔与背景的轮廓仍可辨认，但所有细节（毛发、徽章、面罩反光）全部丢失；同时由于矩形蒙版的"硬截止"，图像四周出现了明显的振铃（Gibbs Ringing），表现为一组同心的明暗波纹。
- 高通结果只保留了高频分量，恰好与低通互补：图像呈现"灰色背景 + 强边缘"的浮雕效果，肩章、头盔下沿、空气罐与背景之间的高对比边缘最为突出；面部光滑区域因为缺乏高频成分而趋于灰色。
...
| 频域低通方案 | MSE | PSNR (dB) |
| --- | --- | --- |
| 矩形蒙版 (half=10) | 633.31 | 20.12 |
| 理想 LPF (D0=40) | 568.51 | 20.58 |
| Butterworth LPF (D0=40, n=2) | 100.89 | 28.09 |
| Gaussian LPF (D0=40) | 52.08 | 30.96 |

可以观察到：

1. 矩形蒙版与理想圆形低通的 PSNR 几乎一样低（≈20 dB），这是它们共有的"硬截止 + 振铃"特征导致的。
2. Butterworth 与 Gaussian 频域低通由于响应连续平滑，PSNR 都大幅提升；其中 Gaussian LPF 与空域 Gaussian Blur 的差异已经接近 31 dB，二者在主观和客观上都几乎不可区分——这与"高斯函数的傅里叶变换还是高斯函数"的数学事实完全吻合。
